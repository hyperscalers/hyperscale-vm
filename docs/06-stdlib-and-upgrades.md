...
| Component | Notes |
|---|---|
| **Fungible resource** | Linear amounts, `delta`/`reserve` native, per-shard supply accumulators (INV-VM-6). Declarative role-gated behaviors — mint, burn, freeze, recall, deposit restriction — with lockable rules; freeze checks ride `locked` mode on cold config, free at scheduling. The highest-value verification target in the system. |
...
| **Badges and auth rules** | Virtual signature badges unify signature and badge authorization; roles use the declarative require/AND/OR/threshold rule language ([05-runtime.md](05-runtime.md) §6); a generic badge-custodian covers admin custody of non-account objects. |
