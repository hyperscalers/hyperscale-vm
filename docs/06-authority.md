...
The algebra is the same on both sides of the declared/stored split. A rule a package *declares* is a `RuleExpr` over expressions, mapped at admission onto the `StoredRule` a gate judges; a rule an account *stores* is a `StoredRule` already. One tree serves both, differing in its leaf alone, so an object whose admins are fixed at publish can say "two of these three" exactly as an account whose keys are stored can — and only the stored form is something claims can be compared against, since asking whether an expression is among those presented compares spellings rather than claims.
