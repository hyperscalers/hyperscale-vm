...
Two consequences follow rather than being defended separately. **A held badge is not delegable**: presenting one goes through a custodial method on the holder's own account, so it is the holder's intent, and their shard's sign-in, that stands behind every use of it. And **proofs have no lifecycle** — no clone, no drop, no runtime proof object. A proof is a manifest edge, so linearity is the graph's property ([02-manifests-and-intents.md](02-manifests-and-intents.md) §1) and the authority graph is fixed when the composition is signed: nothing becomes authorized by something discovered mid-execution.
