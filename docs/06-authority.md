...
| An account's virtual badge | `Claim(principal)` — the account's own address | the attesting set of an intent acting as that account, judged by the account's shard; reaches another intent only by a grant from one that holds it in scope | nobody |
...
| `#[requires(governs(<field>))]` | claims satisfying the rule stored at that field | that field's cell, one of the package's own rule cells | nothing |
...
**`governs(<field>)` is the recovery surface.** It is judged like a declared gate but against the rule stored at the named cell, and it mints nothing, so recovery authority opens recovery methods and nothing else. The field is one of the package's own rule cells — for an account, `recovery` or `veto` — and never the governing cell, which holds a record of two rules judged against keys at sign-in (§5) rather than against claims here. While nothing is stored there, the target's own claim governs — for an account, its virtual badge — which is what lets an account's recovery cells go unwritten until it has something to recover.
...
- **A socket the intent declared and its composer filled**, which is the only way a claim crosses a signing boundary. The declaring intent names the *claim* it is asking for, never whose node supplies it, so what its signer undertook means one thing however it was later carried; the composer fills it from what it holds — a node of its own graph that mints the claim, a socket of its own carrying the same claim, or a grant of an account it acts as. That is scope ([02-manifests-and-intents.md](02-manifests-and-intents.md) §3): an intent grants only a claim it acts as or received, only into its own members' sockets, and a claim reaches a deeper intent by being re-granted at every level by someone who signed that level. Authority never travels upward or sideways — a give carries value alone. A grant of an account is the granter lending its own signature downward: nothing proves it and no node carries it, so the socket it fills waits on nothing.

**The sign-in is the account's shard's, and it lands between the two** (INV-VM-AUTH-6). A signature resolving to a claim on an account is signed content, so admission alone cannot tell whether the keys behind it still open that account. Admission therefore injects, per intent, a read of the account's `auth` cell and one condition: the record stored there admits the attesting keys, or the cell is unwritten and those keys are the address's own. The record is `Authority { primary, confirmation }`, two rules over principal claims, and the verdict is their conjunction: the primary is the everyday rule, the confirmation an optional second factor — a card tapped beside the phone — that is `always()` where there is none, so an account with a card refuses the phone alone. One cell because the sign-in asks one question, and every intent on every account provisions one leaf for it. The condition is judged at materialization by the shard that keeps the cell, before any body of that transaction runs — the same cell the fee reservation reads at inclusion, through one verdict function so the two cannot read it differently.

That cell is the one place in the system judged against *keys*. Every other stored rule names principal addresses and is judged against the accounts a call speaks for; `Claim(bob)` in either rule of Alice's `auth` record is Bob's key, and `Claim(bob)` in Alice's `recovery` is Bob's account. Same vocabulary, two judges, and the cell decides which — which is also why key-based delegation through an `auth` cell is one level deep and never a chain, and why a key its owner retired at their own account still opens another account whose cell names it.
...
An account that never securified has no governing record, and the sign-in admits exactly the key its address derives. Securifying is a one-way door, and the declaration is what makes it one: the write requires the cell absent, so a second securify is refused by the shard holding it before any body runs ([01-effects-and-routing.md](01-effects-and-routing.md) §2). The transition off the address-derived rule happens once, and a caller can see that it will from the signature alone. The two rules that land in that record name principal claims and nothing else, held to it by the parameters' own kind — a leaf naming a resource is one no attesting set could ever meet, so accepting it would store a rule that leaves the account unopenable.

**Rejected: an ambient auth zone.** Per-frame authority accumulating in a mutable container, with barriers deciding how far it propagates, is the alternative, and it fails in two ways that are not fixable from inside it: proofs leak to callees that were never meant to have them, and a proof passed as an argument carries a resource address the caller chose — which is why such a system needs a checked-proof ceremony before a body may read one, and why forgetting the ceremony is a live exploit class. Here a body reads no proof at all. Evidence is resolved by admission from what gates minted and what each composer wired, the resolution is a property of the signed tree, and the three sources above are the whole of it.

Two consequences follow rather than being defended separately. **A held badge is not delegable**: presenting one goes through a custodial method on the holder's own account, so it is the holder's intent, and their shard's sign-in, that stands behind every use of it. And **proofs have no lifecycle** — no clone, no drop, no runtime proof object. A proof is a manifest edge, so linearity is the graph's property ([02-manifests-and-intents.md](02-manifests-and-intents.md) §1) and the authority graph is fixed when the root is signed: nothing becomes authorized by something discovered mid-execution.
...
**A stored rule is neither**; it is a *mutable rule over* badges. An account keeps four: the two factors of its governing record, which are what governs it and the one place judged against keys; and, one cell each, what may replace them after a delay and what may stop a replacement. A component keeps its own under `governs(<field>)`. The rule is the thing that changes; the badges it names are the vocabulary it changes between.
