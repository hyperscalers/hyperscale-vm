...
- **The intent's own signature**, carrying the signer's identity. It reaches only a rule cell under the signer's own prefix: unwritten, the key the address derives from governs it and the sign-in is the whole answer; written, the rule there is judged with the signer's identity among the presented. A signature signs in; whether the key behind it still holds its account's authority is that account's own rule to answer. A rule stored under anyone else's prefix, and an identity a declaration names, take a proof — which is what keeps a key an account has retired from standing in for that account anywhere else.
