...
| [02-manifests-and-intents.md](02-manifests-and-intents.md) | The typed dataflow manifest: nodes, edges, linearity, constraints — and the intent that composes intents through the interface each declares |
...
| [06-authority.md](06-authority.md) | What a proof carries and the two kinds of badge, one rule tree over three leaves, what a method requires and what it mints, the entries a resource's issuer seals into it, the three judgments and where each lands, and choosing between a held badge and a virtual one |
...
| [08-host-integration.md](08-host-integration.md) | The contract between engine and host: read-set provisions, engagement evidence, cross-shard fee assurance, the sign-in an account's shard answers, and work attestation |
| [09-invariants.md](09-invariants.md) | The consolidated register of the VM's properties with stable IDs (INV-VM-*) |

## Conventions

- **Invariant IDs** (`INV-VM-<AREA>-<n>`) are stable references, one family per area — `ACCESS`, `OBJ`, `VALUE`, `RUN`, `AUTH`, `COMP`, `GATE`, `HOST`. They appear inline in each doc where the property arises; the precise statements live once, in [09-invariants.md](09-invariants.md). Cite them rather than restating properties, and keep the argument for a property in the doc that motivates it — an entry states the rule and where it is enforced, not why it holds.
- **The suite boundary.** This suite owns the `INV-VM-*` families and everything about execution semantics, the runtime, and the object model. The host protocol — consensus, atomic cross-shard commitment, sharding, sync, economics — is documented in the hyperscale-rs repository's `docs/`, which owns every other invariant family (`INV-SHARD-*`, `INV-EXEC-*`, `INV-BEACON-*`, …). Where a property spans the boundary — fee assurance, the sign-in — it lives here, in [08-host-integration.md](08-host-integration.md), because it is a requirement the engine's design places on any host.
