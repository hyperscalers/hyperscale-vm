...
- **There is no caller identity, because there is no caller.** A method is named by a manifest node, never by another package's code, so nothing ambient reaches a body and there is no `msg.sender` to synthesize. Authority another party holds is that party's own node in the manifest, presenting its own evidence — which is what makes "only the manager may do this" a badge the manager holds rather than an edge the kernel infers. The composition stays in the signed structure instead of behind it.
...
A body cannot invoke another component: the world imports no call, and every invocation is a manifest node the signer named. Reentrancy is deleted as a concept, not defended against — there is no edge to re-enter along — and the defensive idiom set built around reentrant calls does not carry over.
