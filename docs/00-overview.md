...
- **Authority is presented, never ambient.** Every claim is attested by something that read state — a gate, or the shard keeping the account's own cell — and a node names exactly what it hands its callee, so there is no auth zone to leak through, no `msg.sender` to spoof, and no proof a caller can conjure ([06-authority.md](06-authority.md)). An account's own claim is its shard's to attest, which is what lets a rule name a party once and keep meaning them across every rotation they make.
