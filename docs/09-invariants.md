...
| ID | Class | Property |
|---|---|---|
| **INV-VM-14** | Determinism | **Schedule invariance.** A committed batch's receipts are a pure function of committed content: byte-identical across serial, parallel, and adversarially permuted execution schedules. No environment input reveals schedule position — no intra-block index, no per-execution entropy beyond the per-transaction draw. The kernel takes the clock and the randomness opaquely per transaction; that both are committed content, identical on every participant — the draw derived from committed material rather than local state — is the host's obligation, on the same terms as INV-VM-9 through 11. |
