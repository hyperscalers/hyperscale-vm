...
| **INV-VM-6** | Safety | **Conservation.** For every transaction and every resource, what its value holdings gained equals what they lost, once a mint counts as a loss and a burn as a gain — an amount cell's movements and an instance's presence alike. Checked where the whole transaction is visible, before anything is promoted, so every participant of a cross-shard transaction reaches the same verdict off the same receipt; a transaction that fails it aborts alone, charges nobody, and leaves its batch running. A chain whose every transaction conserves has a conserved stock, genesis included, so the global statement — per-shard totals plus in-flight movements sum to minted-minus-burned — is a consequence of the history rather than a quantity anything keeps. Nothing is stored, split, or attested to hold it, and a shard's own stock is deliberately not answerable in O(1). |
