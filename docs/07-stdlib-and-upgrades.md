...
| Component | Notes |
|---|---|
| **Fungible resource** | Linear amounts, `delta`/`reserve` native, per-shard supply accumulators (INV-VM-VALUE-2). Declarative entries, sealed into the resource's own address — mint, burn, withdraw, deposit, halt, recall — each a rule, and absence withholds; a halt flag is one cell the vocabulary fixes, so the check rides a read the declaration already carries. The highest-value verification target in the system. |
