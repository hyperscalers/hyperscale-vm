...
- **Account auth is the account's own state.** The governing record of two factors, the recovery and veto rules, the delay, and the replacement waiting on it — a recovery proposal or the primary's own amendment — live as owned substates under the account prefix, one cell each; auth checks and recovery operations are same-shard, always. That is also what puts the sign-in where it belongs: the shard keeping an account's governing cell is the one that judges whether the keys attesting an intent may act as it, and it does so before any body runs ([06-authority.md](06-authority.md) §5). A generic badge-custodian component remains for admin custody of non-account objects — rare operations, never the transfer hot path.
- **The deposit fallback is the account's own quarantine**: a `refused` flag per resource, set and cleared by the holder, and a `quarantine` vault beside the main one under the same prefix. A deposit lands in the vault, or in the quarantine where the holder has refused the resource — so a deposit is total for every account, independent of recipient state, local to the recipient's shard, and needs no signature from the recipient. Nothing bonds the bytes a sender lands and no deposit rule governs the vault; what the holder controls is where an unwanted resource waits, and `sweep` is how it leaves. A dApp wanting claim-style distribution builds a sender-side claim component as an ordinary contract; the stdlib blesses no global locker.
...
| Component | Notes |
|---|---|
| **Fungible resource** | Linear amounts, `delta`/`reserve` native, conserved per transaction (INV-VM-VALUE-1). Declarative entries, sealed into the resource's own address — mint, burn, withdraw, deposit, halt, recall — each a rule, and absence withholds; a halt flag is one cell the vocabulary fixes, so the check rides a read the declaration already carries. The highest-value verification target in the system. |
| **Non-fungible resource** | Structural ownership native; per-item leaves; the same behavior set. |
| **Principal (account / persona)** | One primitive, two configurations: an internal auth surface — a governing record of two factors, a recovery rule, a veto rule, a delay, and the recovery proposal or amendment waiting on it — with an optional asset surface. It publishes no method for signing in: an account's own claim is its shard's to attest, so spending and writing name the holder with a claim gate and nothing is composed ahead of them. The governing record's two rules are over principal claims alone, held to it by the parameters' own kind; the recovery and veto cells take the wide vocabulary, so a badge can hold either role. With vaults it is the **account**: virtual (key-derived, lazily instantiated), canonical vault addresses, deferred-bond onboarding and sponsored creation, the kernel claims area as deposit fallback, recovery-exit template host. Without vaults it is the **persona**: a login identity, virtual and free until securified for rotation and recovery; login verification is a client-proven read of its key configuration against an attested root — proof-carrying, no privileged gateway. Personal data stays wallet-side; wallets default to cheap per-context personas, securifying only the reputation-bearing ones. |
| **Badges and auth rules** | A held badge is an ordinary resource, fungible or with one instance per holder, presented through a custodial method on the holder's own account; an account's own badge is virtual, held by nobody and attested by its shard. Rules are thresholds over the claims a call presents, declared by a package or stored by an account under one algebra ([06-authority.md](06-authority.md)). |
...
Stdlib verification obligations attach to the invariants they carry (INV-VM-VALUE-1 conservation, INV-VM-VALUE-2 linearity) rather than minting new IDs per component; the register entry for each names the components that discharge it.
