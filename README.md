...
- `crates/vm-effects` — effect signatures: the access DSL and its evaluator, the content-addressed metadata cache, and `route()`, the fold from a manifest to per-shard effect sets and the static call graph.
