...
- **A frozen deterministic profile, executed twice.** A core WASM module under an allowlisted subset — no floats, no threads, metered by a pass the module carries in its own bytes, down to boundary copies — run by a version-pinned blessed engine (wasmtime) and an independently written reference interpreter, differentially tested against each other. Divergence is a release blocker, whichever side is wrong.
...
`guests/` is the pinned-toolchain guest workspace: the protocol's own packages and every fixture package the corpus executes on both runtimes, seventeen crates under one lockfile and one profile. The committed stdlib blobs are canonically Linux-built: roll them with `scripts/regenerate-stdlib.sh` (Docker, the same x86-64 Linux CI verifies on) and commit the result, not a local build from another OS.
