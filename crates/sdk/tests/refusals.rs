...
fn the_lowering_refuses_what_it_would_declare_wrongly() {
    ...
    refuse.compile_fail("tests/refusals/loop_break_output.rs");
    refuse.compile_fail("tests/refusals/hidden_edge.rs");
    ...
    refuse.compile_fail("tests/refusals/filed_into_foreign.rs");
    refuse.compile_fail("tests/refusals/vault_holds_non_fungible.rs");
    refuse.compile_fail("tests/refusals/instances_hold_fungible.rs");
    refuse.compile_fail("tests/refusals/reserved_underscore_param.rs");
...
/// A component's self-proof is conditional or it is no gate at all: a
/// `#[proves(self)]` body that cannot decline hands the component's
/// claim to whoever calls it first. The account's is different — the
/// kernel judges its stored rule before the export runs — so a
/// principals package stays free to prove with an empty body.
#[test]
fn the_lowering_refuses_a_prover_that_cannot_decline() {
    let refuse = TestCases::new();
    refuse.compile_fail("tests/refusals/unconditional_prover.rs");
}
...
fn the_macro_bounds_what_a_helper_may_be() {
    ...
    // trait's, so nothing splices them and a shared name does not shadow
    // the inherent helper.
