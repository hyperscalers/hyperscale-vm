//! The hashing seam.
//!
//! Canonical child addresses, fresh IDs, and manifest hashes all flow through
//! one trait so this crate never binds a concrete hash function; the protocol
//! hash is supplied at integration. Every derivation is domain-separated and
//! part-framed, so a hasher swap changes values but never meanings.
