...
//! spelling — `grants(withdraw = held(issued(Registered)))` — where every
//! other fixture here builds a [`ResourceMeta`] by hand. What that
...
fn an_authored_rule_governs_a_holder_the_package_never_named() {
    ...
    // One leaf, not an interval: every injected presence read on the
    // transfer path is a point, and a behaviour landing later may not
    // widen it into a scan.
    let injected = declaration
        ...
        .count();
    assert_eq!(injected, 1, "one credential, asked once");
}
...
fn the_guest_grants_what_these_cases_are_about() {
    ...
    assert_eq!(
        granted,
        BTreeSet::from([GrantedBehaviour::Withdraw, GrantedBehaviour::Recall]),
        "a withdraw entry to enforce and an authority entry to stay plain beside it",
    );
}
