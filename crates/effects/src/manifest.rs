...
pub enum JudgedLeaf {
    ...
    /// against the claims the call presents.
    ///
    /// An unwritten cell holds no rule, and no rule admits nobody: what
    /// governs a cell before anything is written there is the package's
    /// own business, declared as a branch beside this leaf. Keys never
    /// enter — a rule stored here names principal addresses and is
    /// judged against the accounts the call speaks for. The one cell
    /// judged against keys is an account's `auth`, and [`Self::Signed`]
    /// is the leaf that reads it.
    Stored {
