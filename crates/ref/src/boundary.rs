...
fn resolve<H: KernelHost>(module: &str, name: &str) -> Option<Dispatch<H>> {
    ...
    Some(match (module, name) {
        ...
        (STATE, "site_len") => dispatch!(site_len(0) -> value),
        (STATE, "site_declared") => dispatch!(site_declared(0, 1) -> value),
        (STATE, "site_get") => dispatch!(site_get(0, 1) -> value),
        (STATE, "site_set") => dispatch!(site_set(0, 1, 2, 3)),
        (STATE, "site_seal") => dispatch!(site_seal(0, 1)),
        (STATE, "site_open_seal") => dispatch!(site_open_seal(0, 1, 2) -> value),
        (STATE, "site_clear") => dispatch!(site_clear(0, 1)),
        (STATE, "site_balance") => dispatch!(site_balance(0, 1, 2)),
        (STATE, "site_take") => dispatch!(site_take(0, 1, 2) -> value),
        (STATE, "site_put") => dispatch!(site_put(0, 1, 2)),
        (STATE, "site_reserve_take") => dispatch!(site_reserve_take(0, 1) -> value),
        (STATE, "site_count") => dispatch!(site_count(0, 1) -> value),
        (STATE, "site_covered") => dispatch!(site_covered(0, 1) -> value),
        (STATE, "site_order") => dispatch!(site_order(0, 1, 2, 3)),
        (STATE, "site_entry") => dispatch!(site_entry(0, 1, 2) -> value),
        (STATE, "site_entry_set") => dispatch!(site_entry_set(0, 1, 2, 3, 4)),
        (STATE, "site_insert") => dispatch!(site_insert(0, 1, 2, 3, 4)),
        (STATE, "site_remove") => dispatch!(site_remove(0, 1, 2)),
        (STATE, "site_instance_take") => dispatch!(site_instance_take(0, 1, 2, 3) -> value),
        (STATE, "site_instance_put") => dispatch!(site_instance_put(0, 1, 2, 3, 4)),
        (STATE, "bucket_take") => dispatch!(bucket_take(0, 1) -> value),
        (STATE, "bucket_split") => dispatch!(bucket_split(0, 1, 2) -> value),
        (STATE, "bucket_put") => dispatch!(bucket_put(0, 1)),
        (STATE, "bucket_amount") => dispatch!(bucket_amount(0, 1)),
        (STATE, "bucket_drop") => dispatch!(bucket_drop(0)),
        (STATE, "mint") => dispatch!(mint(0, 1) -> value),
        (STATE, "mint_instances") => dispatch!(mint_instances(0, 1, 2) -> value),
        (STATE, "burn") => dispatch!(burn(0)),
        (MATH, "mul_div") => dispatch!(mul_div(0, 1, 2, 3, 4)),
        (MATH, "geometric_mean") => dispatch!(geometric_mean(0, 1, 2)),
        (MATH, "fraction_compose") => dispatch!(fraction_compose(0, 1, 2, 3, 4, 5)),
        (MATH, "fraction_cmp") => dispatch!(fraction_cmp(0, 1, 2, 3) -> value),
        (MATH, "fixed_pow") => dispatch!(fixed_pow(0, 1, 2, 3)),
...
impl<'m, H: KernelHost> RefModuleInstance<'m, H> {
    ...
    fn call(&mut self, export: &str, args: &[GuestArg<'_>]) -> Invoked {
        ...
        let Ok((values, registers)) = abi::place(args) else {
