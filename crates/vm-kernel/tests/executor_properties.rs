...
/// construction. A snapshot is, because it pins to the batch baseline both
/// sides carry.
#[derive(Clone, Copy, Debug)]
enum PortableClaim {
    Snapshot(u8),
...
fn arb_portable_claim() -> impl Strategy<Value = PortableClaim> {
    prop_oneof![
        (0u8..CELLS).prop_map(PortableClaim::Snapshot),
...
fn portable_declared(claims: &[PortableClaim]) -> EffectSet {
    ...
    for claim in claims {
        let effect = match *claim {
            PortableClaim::Snapshot(k) => Effect {
                target: EffectTarget::Point(cell(k)),
                mode: Mode::Snapshot {
                    window: Window::Bounded(4),
                },
...
/// alone. What it snapshots reaches the receipt through the return value,
...
fn portable_runner() -> impl Fn(TxHash, KernelSession) -> RunResult + Sync {
    move |id, mut session: KernelSession| {
        ...
        for (rep, capability) in caps.iter().enumerate() {
            ...
            match capability {
                Capability::Snapshot(_) => {
                    let cell = session.snap_cell(rep).unwrap_or_default();
                    observed =
